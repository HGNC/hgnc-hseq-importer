...
from hgnc_hseq_importer.cli import main
from hgnc_hseq_importer.exceptions import ConfigError, ServiceError


class TestMainSuccess:
    def test_main_returns_zero_on_success(self):
        with patch("hgnc_hseq_importer.cli.configure_logging") as mock_log, \
             patch("hgnc_hseq_importer.cli.Settings") as mock_settings_cls, \
             patch("hgnc_hseq_importer.cli.MainService") as mock_svc_cls:
    ...
    def test_main_loads_settings(self):
        with patch("hgnc_hseq_importer.cli.configure_logging"), \
             patch("hgnc_hseq_importer.cli.Settings") as mock_settings_cls, \
             patch("hgnc_hseq_importer.cli.MainService"):
            mock_settings_cls.return_value = MagicMock()
            main([])
...
class TestMainConfigError:
    def test_config_error_returns_exit_code_2(self):
        with patch("hgnc_hseq_importer.cli.configure_logging"), \
             patch("hgnc_hseq_importer.cli.Settings", side_effect=ConfigError("bad config")):
            result = main([])
        assert result == 2

    def test_config_error_during_settings_load(self):
        with patch("hgnc_hseq_importer.cli.configure_logging"), \
             patch("hgnc_hseq_importer.cli.Settings", side_effect=ConfigError("missing env")):
            result = main([])
        assert result == 2
...
class TestMainNoBusinessLogic:
    def test_main_does_not_import_repositories_directly(self):
        ...
        import hgnc_hseq_importer.cli as cli_module
